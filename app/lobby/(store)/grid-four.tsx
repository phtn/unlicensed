...
export function GridItem({
  ...
  return (
    <motion.article
      ...
      className={cn(
        'group relative isolate overflow-hidden md:pt-1 md:pl-1 mb-1 _shadow-[0_24px_80px_-48px_rgba(0,0,0,0.2)] transition-transform duration-300',
        'border border-r-0 border-b-0 border-foreground/10 bg-background dark:bg-dark-table bg-linear-to-r from-background/20 dark:from-dark-table via-brand/30 to-background',
        className,
      )}
      {...props}>
      ...
      <div className='rounded-xs dark:bg-background/80 bg-background px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-6 h-full'>
        ...
        <div className='relative z-10 md:ml-4 md:mt-4 gap-4 md:gap-8 flex items-start'>
          ...
          <div>
            ...
            <p className='md:mt-3 mt-1 max-w-[54ch] md:max-w-[45ch] text-sm leading-6 text-muted-foreground'>
              {item.description}
            </p>
          </div>
        </div>
