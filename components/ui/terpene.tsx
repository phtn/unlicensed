    ...
    <Chip
      ...
      startContent={
        <Icon
          ...
          className={cn(' size-5 aspect-square', {
      ...
      classNames={{
        base: `flex justify-between pl-1 h-7 grow-0 bg-linear-to-r from-slate-600/5 via-transparent to-transparent overflow-hidden`,
        content: 'font-light text-foreground capitalize text-sm',
      }}
      variant='flat'>
      {name}
    ...
    <Chip
      ...
      startContent={
        name ? (
          ...
          <span className='text-sm font-space text-slate-600 font-semibold'>
            {label}
          </span>
        )
      }
      ...
      variant='flat'>
      <span className={cn('ml-1.5 font-space opacity-100')}>{value}</span>
    </Chip>
  )
}
