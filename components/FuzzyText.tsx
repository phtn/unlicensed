...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  children,
  fontSize = 'clamp(0.5rem, 0.3vw, 1rem)',
  fontWeight = 500,
  fontFamily = 'inherit',
  color = '#fff',
  enableHover = true,
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const extraWidthBuffer = compact ? 0 : 10
      const offscreenWidth = textBoundingWidth + extraWidthBuffer

      offscreen.width = offscreenWidth
      offscreen.height = tightHeight

      const xOffset = compact ? 0 : extraWidthBuffer / 2
      offCtx.font = `${fontWeight} ${fontSizeStr} ${computedFontFamily}`
      offCtx.textBaseline = 'alphabetic'
      ...
      const horizontalMargin = compact ? 0 : fuzzRange + 20
